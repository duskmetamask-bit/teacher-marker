...
// Simple markdown-ish renderer: bold, bullet lists, headings, line breaks
function renderMarkdown(text: string): React.ReactNode {
  ...
  lines.forEach((line, i) => {
    ...
    // Horizontal rule
      ...
      elements.push(
        <hr key={i} style={{ borderColor: C.border, marginTop: 8, marginBottom: 8 }} />
      );
    // Bullet
    } else if (line.startsWith("- ") || line.startsWith("• ")) {
      elements.push(
    ...
    // Numbered
      ...
      if (match) {
        elements.push(
          <p key={i} style={{ color: C.text2, margin: "2px 0", paddingLeft: 16, fontSize: 13 }}>
            <span style={{ color: C.primaryHover, fontWeight: 600 }}>{match[1]}.</span>{" "}
            {applyInline(match[2])}
          </p>
        );
      }
...
export default function ChatInterface({ teacherProfile, sessionId }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    ...
    if (teacherProfile?.name) {
      const greeting = `Hi ${teacherProfile.name}! 👋 I'm **PickleNickAI**, your personal teaching assistant.\n\nI know you teach **${teacherProfile.yearLevels.join(", ")}** and cover **${teacherProfile.subjects.join(", ")}**.\n\nWhat can I help you with today? You can ask me for:\n- Lesson plans (WIEP format)\n- Assessment rubrics\n- Worksheets\n- Unit planners\n- Differentiation strategies`;
      setMessages([{ role: "assistant", content: greeting }]);
    } else {
      setMessages([{
        role: "assistant",
        content: "Hi! I'm **PickleNickAI**, your teaching assistant. What can I help you with today? Ask me for lesson plans, rubrics, worksheets, or any teaching resource!",
      }]);
    }
  }, []);
  ...
  return (
    <div
      style={{ background: C.bg, display: "flex", flexDirection: "column", height: "100dvh" }}
    >
      {/* Header */}
      <header
        style={{
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <h1 style={{ color: C.text, fontSize: 16, fontWeight: 800, margin: 0 }}>
                PickleNickAI
              </h1>
              {teacherProfile?.name && (
                <span
...
{(teacherProfile?.yearLevels?.length ?? 0) > 0 && (
              <p style={{ color: C.text3, fontSize: 11, margin: 0, marginTop: 2 }}>
                Teaching: {(teacherProfile!.yearLevels ?? []).slice(0, 4).join(", ")}
                {(teacherProfile!.yearLevels ?? []).length > 4 ? " +" + ((teacherProfile!.yearLevels ?? []).length - 4) + " more" : ""}
              </p>
            )}
          </div>
          <a
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
          {messages.map((msg, i) => (
            ...
            >
              ...
              >
                {msg.role === "user" ? (
                  <p style={{ color: "#fff", fontSize: 13, margin: 0, whiteSpace: "pre-wrap" }}>
                    {msg.content}
                  </p>
                ) : (
      ...
      >
        ...
        >
          <textarea
            ...
            placeholder="Ask me for a lesson plan, rubric, worksheet… (Enter to send, Shift+Enter for new line)"
            rows={2}
            style={{
              flex: 1,
              background: C.surface2,
        ...
        <p style={{ color: C.text3, fontSize: 10, textAlign: "center", marginTop: 6, margin: "6px 0 0" }}>
