...
function renderMarkdown(text: string): React.ReactNode {
  ...
  lines.forEach((line, i) => {
    if (line.startsWith("### ")) {
      elements.push(<p key={i} style={{ color: "var(--primary)", fontWeight: 700, marginTop: 12, marginBottom: 4, fontSize: 13 }}>{applyInline(line.slice(4))}</p>);
    } else if (line.startsWith("## ")) {
      elements.push(<p key={i} style={{ color: "var(--text)", fontWeight: 800, marginTop: 14, marginBottom: 4, fontSize: 14 }}>{applyInline(line.slice(3))}</p>);
    } else if (line.startsWith("# ")) {
      elements.push(<p key={i} style={{ color: "var(--text)", fontWeight: 900, marginTop: 16, marginBottom: 6, fontSize: 15 }}>{applyInline(line.slice(2))}</p>);
    } else if (line.startsWith("---")) {
      ...
      if (match) elements.push(<p key={i} style={{ color: "var(--text-secondary)", margin: "2px 0", paddingLeft: 16, fontSize: 13 }}><span style={{ color: "var(--primary)", fontWeight: 600, marginRight: 4 }}>{match[1]}.</span> {applyInline(match[2])}</p>);
    } else if (line.trim() === "") {
      elements.push(<div key={i} style={{ height: 6 }} />);
    } else {
      elements.push(<p key={i} style={{ color: "var(--text-secondary)", margin: "2px 0", fontSize: 13, lineHeight: 1.6 }}>{applyInline(line)}</p>);
    }
  });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: "var(--background)" }}>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {teacherProfile?.name && (
                <Badge variant="primary">{teacherProfile.name}</Badge>
              )}
            </div>
            {(teacherProfile?.yearLevels?.length ?? 0) > 0 && (
              <p style={{ color: "var(--text-secondary)", fontSize: 11, margin: 0, marginTop: 2 }}>
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {messages.length === 0 && !loading && (
            <div style={{ padding: "1rem" }}>
              <p style={{ color: "var(--text-secondary)", fontSize: 12, marginBottom: 12, textAlign: "center" }}>
      ...
      <div style={{
        ...
        <p style={{ color: "var(--text-secondary)", fontSize: 10, textAlign: "center", margin: "6px 0 0" }}>
